...
var parseTransformString = require('./parseTransformString');
...
XCSSMatrix.prototype.inverse = function () {
    ...
    // `adjoint` doesn't return an `XCSSMatrix`
    // just returns an object with the `m*` properties
    var adjoint = utils.matrix.adjoint(this);
    var result  = new XCSSMatrix();
    // copy the `m*` properties from `adjoint` to `result`
    points3d.forEach(function (point) {
        result[point] = adjoint[point];
    });
...
function toMatrixString(transformString) {
    var transformOperations = parseTransformString(transformString);
